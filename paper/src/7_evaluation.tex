...
\subsection{シナリオ}
基本的なSIIT-DCネットワークの構築を想定し，IPv4提供サービスサーバ群を起動した後に，BR間のEAMTの一貫性が正しく担保されるかを検証する．
...
しかしながら本評価環境においては，ルートリフレクタが収容できるBGPコネクションに限りがあり，ルートリフレクタが保有するBGPコネクションが一定の値を超えた場合に，全てのBRのEAMTが維持できなくなる課題が明らかになった．
...
本評価実験の結果のうち，「IPv4サービス提供サーバの追加」を図\ref{fig:eval2_result_add}に，「IPv4サービス提供サーバの削除」を図\ref{fig:eval2_result_add}に示す．両図では，各説明変数$x$での試行の分布と，それぞれの平均の推移を表している．

はじめに本評価実験の概況について考察する．．
いずれのケース及び説明変数$x$に於いても，各BRのEAMTが平均して概ね10秒程度で収束していることがわかる．本評価実験の結果から，本実験環境において検証可能なIPv4サービス提供サーバ数の範囲内において，サーバ数の嘉多に関わらず，変更追従機構が効果的に作用していることが評価できる．


次に，各ケースにおける経過時間の推移について述べる．
IPv4サービス提供サーバの削除のケースでは，初期状態のIPv4サービス提供サーバ数$x$に関わらずEAMTの収束時間はほぼ横ばいに推移している．一方で，サーバを追加したケースでは$x$が大きくなるに従って，EAMTの収束に掛かる時間が増なだらかに増加傾向にあることが読み取れる．
